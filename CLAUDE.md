...
`internal/admin/handlers_system_{version,upgrade}.go` + `internal/deployer/grpcserver/upgrade.go` implement a Coolify-style one-click upgrade. `GET /api/system/version/latest` hits GHCR with an anonymous manifest HEAD (5 min cache) to surface the `:latest` digest. `POST /api/system/upgrade` calls into the deployer over gRPC (`SystemUpgrade` server-streaming RPC), which: takes the in-process mutex (409 on concurrent), writes the target tag into `/host/muvon/.env`, runs `pg_dump -Fc` inside the postgres container via `ContainerExecCapture`, and spawns a short-lived `docker:27-cli` helper container (`RunHelperContainer` in `internal/deployer/helpers.go`) that bind-mounts the Docker socket plus `/opt/muvon` and runs `compose pull && compose up -d --wait`. Helper stdout/stderr is demuxed and routed via `upgradeBroker` → SSE to `GET /api/system/upgrade/stream`. The helper recreates the deployer itself, so the gRPC stream EOF is treated as `done`. Required mounts in `docker-compose.yml`: `/var/run/docker.sock`, `/opt/muvon:/host/muvon:rw`, and the `backups` volume.
