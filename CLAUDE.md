...
- **pg_search** (ParadeDB/Tantivy) is created by the first migration and must be installed for startup to succeed, but **nothing queries it any more**. Its BM25 operator did not propagate from a hypertable to its chunks, so the `drop_dialog_bm25_index` migration replaced it with `pg_trgm` trigram GIN indexes on path, host, user_agent, client_ip, `user_identity::text` and the body columns. Log search is `ILIKE '%term%'` against those. Do not describe the product as having BM25 search, and do not remove the extension without also removing the `CREATE EXTENSION` line.
