...
// pushBackoff dictates how aggressively a failed push retries. Certs
// are precious — agent already serves them locally, but losing the
// central backup means a future agent migration starts from scratch.
// Exponential up to ~30 min, then keeps trying every 30 min until the
// process exits or the push succeeds.
...
// PushCertificate uploads a freshly-issued ACME certificate to central
// as a backup. Errors retry in the background with exponential backoff
// up to ~30 minutes between attempts; the caller never blocks. The
// cert is already served locally — missing the central backup is a
// degraded mode, not a fatal one.
func (s *AgentCertSync) PushCertificate(domain string, certKeyPEM []byte) {
	...
	// Run in the background so the autocert write path never blocks on
	// network. Exponential backoff with a permanent floor at the last
	// entry — keep trying as long as the process is alive.
	go func() {
		for attempt := 0; ; attempt++ {
			d := pushBackoff[len(pushBackoff)-1]
			...
			if s.tryPushOnce(domain, body) {
				if attempt > 0 {
					slog.Info("agent cert backup pushed after retry", "domain", domain, "attempt", attempt+1)
				} else {
...
// tryPushOnce makes one POST attempt and reports success/failure so the
// retry loop can compose the strategy. Errors are logged here, never
// surfaced — by the time the caller has scheduled the goroutine they
// no longer have an error channel to honour.
func (s *AgentCertSync) tryPushOnce(domain string, body []byte) bool {
	ctx, cancel := context.WithTimeout(context.Background(), agentCertTimeout)
	defer cancel()
	...
	resp, err := s.httpClient.Do(req)
	if err != nil {
		slog.Debug("agent cert push attempt failed", "domain", domain, "error", err)
		return false
	}
	defer resp.Body.Close()
	if resp.StatusCode >= 500 {
		...
		slog.Debug("agent cert push got 5xx; will retry", "domain", domain, "status", resp.StatusCode, "body", string(bs))
		return false
	}
	if resp.StatusCode >= 400 {
		// 4xx is permanent for the same payload — bail out instead of
		// hammering central with a request it'll never accept.
		bs, _ := io.ReadAll(resp.Body)
		slog.Warn("agent cert push rejected (no retry)", "domain", domain, "status", resp.StatusCode, "body", string(bs))
		return true
	}
	return true
}
