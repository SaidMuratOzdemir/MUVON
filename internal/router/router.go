...
func New(ch *config.Holder, logSink proxy.LogSink, transport http.RoundTripper, hm *health.Manager, instanceTracker proxy.InstanceTracker, frontendFS fs.FS, adminDomain string, adminHandler http.Handler) *Router {
	return &Router{
		proxyHandler: proxy.NewHandler(ch, logSink, transport, hm, instanceTracker),
		configHolder: ch,
