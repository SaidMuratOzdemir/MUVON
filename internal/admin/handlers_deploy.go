...
func (s *Server) handleRollbackProject(w http.ResponseWriter, r *http.Request) {
	...
	// enqueue path used by manual / webhook deploys.
	dr := deployRequest{
		Project:    projectSlug,
		ReleaseID:  "rollback-" + target.ReleaseID + "-" + strconv.FormatInt(time.Now().Unix(), 10),
		Repo:       target.Repo,
		Branch:     target.Branch,
		CommitSHA:  target.CommitSHA,
		Components: make(map[string]deployComponentInput, len(target.Components)),
	}
	for _, c := range target.Components {
		dr.Components[c.Slug] = deployComponentInput{ImageRef: c.ImageRef, ImageDigest: c.ImageDigest}
	}
	normalized, _ := json.Marshal(dr)
	deployment, idempotent, err := s.enqueueDeployRequest(r, dr, normalized, "rollback")
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": err.Error()})
		return
	}
