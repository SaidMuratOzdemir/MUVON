...
type DeployComponent struct {
	...
	// EnvSecretKeys names the keys in Env whose values are stored as
	// "enc:"-prefixed ciphertext (secret.Box). UI masks these on read;
	// deployer decrypts them when starting a container.
	EnvSecretKeys []string `json:"env_secret_keys"`
	Mounts        []Mount  `json:"mounts"`
	IsRoutable    bool     `json:"is_routable"`
	// AgentID names which agent host runs this component's containers.
	// Empty string ("") means central — muvon-deployer on the operator's
...
// deploymentSelectCols lists every column scanDeployment expects, joining
// deploy_projects p and deploy_releases r to recover the human-friendly
// project_slug and release/repo metadata stored elsewhere.
const deploymentSelectCols = `d.id::text, d.project_id, p.slug, d.release_uuid::text, d.release_id,
	r.repo, r.branch, r.commit_sha, d.trigger, d.status, COALESCE(d.agent_id, ''),
	d.payload, d.error, d.started_at, d.finished_at, d.created_at, d.updated_at`

func scanDeployment(scan func(...any) error) (Deployment, error) {
	var dep Deployment
	err := scan(
		&dep.ID, &dep.ProjectID, &dep.ProjectSlug, &dep.ReleaseUUID, &dep.ReleaseID,
		&dep.Repo, &dep.Branch, &dep.CommitSHA, &dep.Trigger, &dep.Status, &dep.AgentID,
		&dep.Payload, &dep.Error, &dep.StartedAt, &dep.FinishedAt, &dep.CreatedAt, &dep.UpdatedAt,
	)
	return dep, err
}

// deployComponentSelectCols lists every column scanDeployComponent
// expects, prefixed with `c.` for joining against deploy_projects p.
const deployComponentSelectCols = `c.id, c.project_id, p.slug, c.slug, c.name, c.source_repo, c.image_repo,
...
func scanDeployComponent(scan func(...any) error) (DeployComponent, error) {
	...
	if c.EnvSecretKeys == nil {
		c.EnvSecretKeys = []string{}
	}
	return c, nil
}
...
func (d *DB) CreateDeployProject(ctx context.Context, slug, name, sourceRepo, webhookSecret string) (DeployProject, error) {
	p, err := scanDeployProject(d.Pool.QueryRow(ctx,
		`INSERT INTO deploy_projects (slug, name, source_repo, webhook_secret)
		 VALUES ($1, $2, $3, $4)
		 RETURNING id, slug, name, source_repo, webhook_secret, is_active, created_at, updated_at`,
		slug, name, sourceRepo, webhookSecret,
	).Scan)
	if err != nil {
		return p, fmt.Errorf("create deploy project: %w", err)
	}
	return p, nil
}

func (d *DB) DeleteDeployProject(ctx context.Context, slug string) error {
	tag, err := d.Pool.Exec(ctx, `DELETE FROM deploy_projects WHERE slug = $1`, slug)
	if err != nil {
		return fmt.Errorf("delete deploy project: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("delete deploy project: not found")
	}
	return nil
}
...
type DeployComponentInput struct {
	...
	Mounts                  []Mount
	IsRoutable              bool
	// AgentID = "" → central host, non-empty → edge agent. Set only at
	// create time; switching hosts after the first deploy would orphan
	// running containers on the original side.
	AgentID      string
	Paused       bool
	KeepReleases int
}
...
func (d *DB) GetDeployComponent(ctx context.Context, projectSlug, componentSlug string) (DeployComponent, error) {
	c, err := scanDeployComponent(d.Pool.QueryRow(ctx,
		`SELECT `+deployComponentSelectCols+`
		 FROM deploy_components c
		 JOIN deploy_projects p ON p.id = c.project_id
		 WHERE p.slug = $1 AND c.slug = $2`, projectSlug, componentSlug).Scan)
	if err != nil {
		return c, fmt.Errorf("get deploy component: %w", err)
	}
	return c, nil
}

func (d *DB) CreateDeployComponent(ctx context.Context, in DeployComponentInput) (DeployComponent, error) {
	envJSON, err := json.Marshal(in.Env)
	if err != nil {
		return DeployComponent{}, fmt.Errorf("create deploy component env marshal: %w", err)
	}
	mountsJSON, err := json.Marshal(in.Mounts)
	if err != nil {
		return DeployComponent{}, fmt.Errorf("create deploy component mounts marshal: %w", err)
	}
	...
	c, err := scanDeployComponent(d.Pool.QueryRow(ctx,
		`WITH ins AS (
		    INSERT INTO deploy_components (
		        project_id, slug, name, source_repo, image_repo, internal_port,
		...
		in.HealthPath, in.HealthExpectedStatus, in.MigrationCommand,
...
func (d *DB) UpdateDeployComponent(ctx context.Context, projectSlug, componentSlug string, in DeployComponentInput) (DeployComponent, error) {
	envJSON, err := json.Marshal(in.Env)
	if err != nil {
		return DeployComponent{}, fmt.Errorf("update deploy component env marshal: %w", err)
	}
	mountsJSON, err := json.Marshal(in.Mounts)
	if err != nil {
	...
	c, err := scanDeployComponent(d.Pool.QueryRow(ctx,
		`WITH upd AS (
		    ...
		    SET name = $3,
		        ...
		        internal_port = $6,
		        health_path = $7,
		        health_expected_status = $8,
		        migration_command = $9,
		        restart_retries = $10,
		        drain_timeout_seconds = $11,
		        long_drain_timeout_seconds = $12,
		        networks = $13,
		        env_file_path = $14,
		        env = $15,
		        mounts = $16,
		    ...
		    RETURNING c.id, c.project_id, c.slug, c.name, c.source_repo, c.image_repo, c.internal_port,
...
func (d *DB) GetDeployComponentAgentID(ctx context.Context, componentID int) (string, error) {
	var agentID *string
	err := d.Pool.QueryRow(ctx,
		`SELECT agent_id FROM deploy_components WHERE id = $1`, componentID,
	).Scan(&agentID)
	if err != nil {
		return "", fmt.Errorf("get deploy component agent: %w", err)
	}
	if agentID == nil {
		return "", nil
	}
...
// GetDeployInstanceAgentID resolves an instance's owning agent by joining
// through its component.
func (d *DB) GetDeployInstanceAgentID(ctx context.Context, instanceID string) (string, error) {
	var agentID *string
	err := d.Pool.QueryRow(ctx,
		`SELECT c.agent_id
	...
	if err != nil {
		return "", fmt.Errorf("get deploy instance agent: %w", err)
	}
	if agentID == nil {
		return "", nil
	}
...
func (d *DB) DeleteDeployComponent(ctx context.Context, projectSlug, componentSlug string) error {
	tag, err := d.Pool.Exec(ctx,
		`DELETE FROM deploy_components c
		 USING deploy_projects p
		 WHERE c.project_id = p.id AND p.slug = $1 AND c.slug = $2`,
		projectSlug, componentSlug)
	if err != nil {
		return fmt.Errorf("delete deploy component: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("delete deploy component: not found")
	}
	return nil
}
...
func (d *DB) EnqueueDeployment(ctx context.Context, in EnqueueDeploymentInput) (Deployment, bool, error) {
	...
	// A deployment runs on exactly one host (central or one specific
	// agent). Components in the same project are usually colocated, but
	// we still defend against an operator who edits one component's
	// agent_id without the others — reject the deployment rather than
	// silently splitting it across hosts.
	deploymentAgent := ""
	agentSeen := false
	for _, component := range in.Components {
		...
		if err := tx.QueryRow(ctx,
			`SELECT id, agent_id, paused FROM deploy_components WHERE project_id = $1 AND slug = $2`,
			project.ID, component.Slug,
		).Scan(&componentID, &agentID, &paused); err != nil {
			return Deployment{}, false, fmt.Errorf("enqueue deployment component %s: %w", component.Slug, err)
		}
		if paused {
			return Deployment{}, false, fmt.Errorf("enqueue deployment: component %s is paused; resume it before deploying", component.Slug)
		}
		thisAgent := ""
		if agentID != nil {
			thisAgent = *agentID
		...
		if !agentSeen {
			deploymentAgent = thisAgent
			agentSeen = true
		} else if thisAgent != deploymentAgent {
			return Deployment{}, false, fmt.Errorf("enqueue deployment: components straddle hosts (component %s on %q, others on %q) — move them to the same host before deploying", component.Slug, thisAgent, deploymentAgent)
		}
	...
	var deploymentAgentArg any
	if deploymentAgent != "" {
		deploymentAgentArg = deploymentAgent
	}
	...
	if err := tx.QueryRow(ctx,
		`WITH ins AS (
		    INSERT INTO deployments (project_id, release_uuid, release_id, trigger, status, payload, agent_id)
		    VALUES ($1, $2, $3, $4, 'pending', $5, $6)
		    ON CONFLICT (project_id, release_uuid) DO UPDATE
		      SET status = 'pending', started_at = NULL, finished_at = NULL,
		          updated_at = now(), payload = EXCLUDED.payload, agent_id = EXCLUDED.agent_id
		      WHERE deployments.status IN ('succeeded', 'failed')
		    RETURNING id::text, project_id, release_uuid::text, release_id, trigger, status, COALESCE(agent_id, '') AS agent_id, payload, error, started_at, finished_at, created_at, updated_at, true AS inserted
		 )
		 SELECT id, project_id, release_uuid, release_id, trigger, status, agent_id, payload, error, started_at, finished_at, created_at, updated_at, inserted
		 FROM (
		     SELECT id::text, project_id, release_uuid::text, release_id, trigger, status, COALESCE(agent_id, '') AS agent_id, payload, error, started_at, finished_at, created_at, updated_at, inserted
		     FROM ins
		     UNION ALL
		     SELECT id::text, project_id, release_uuid::text, release_id, trigger, status, COALESCE(agent_id, '') AS agent_id, payload, error, started_at, finished_at, created_at, updated_at, false AS inserted
		     FROM deployments
		     WHERE project_id = $1 AND release_uuid = $2
		 ) q
		 ORDER BY inserted DESC
		 LIMIT 1`,
		project.ID, releaseUUID, in.ReleaseID, in.Trigger, in.Payload, deploymentAgentArg,
	).Scan(&dep.ID, &dep.ProjectID, &dep.ReleaseUUID, &dep.ReleaseID, &dep.Trigger, &dep.Status, &dep.AgentID, &dep.Payload, &dep.Error, &dep.StartedAt, &dep.FinishedAt, &dep.CreatedAt, &dep.UpdatedAt, &inserted); err != nil {
...
// ClaimNextDeployment picks the oldest pending deployment whose agent_id
// matches the caller's owner. agentID == "" claims central deployments
// (rows with NULL agent_id); a non-empty agentID claims only that
// agent's edge deployments. ProjectSlug/Repo/Branch/CommitSHA are not
// returned here — callers that need them call LoadDeploymentPlan next.
func (d *DB) ClaimNextDeployment(ctx context.Context, agentID string) (Deployment, bool, error) {
	var (
		filter string
		args   []any
	)
	...
	query := `WITH next AS (
		 ...
		 FROM next
		 WHERE d.id = next.id
		 RETURNING d.id::text, d.project_id, ''::text AS project_slug, d.release_uuid::text, d.release_id,
		           ''::text AS repo, ''::text AS branch, ''::text AS commit_sha,
		           d.trigger, d.status, COALESCE(d.agent_id, ''),
		           d.payload, d.error, d.started_at, d.finished_at, d.created_at, d.updated_at`
	dep, err := scanDeployment(d.Pool.QueryRow(ctx, query, args...).Scan)
...
func (d *DB) LoadDeploymentPlan(ctx context.Context, deploymentID string) (DeploymentPlan, error) {
	...
	err := d.Pool.QueryRow(ctx,
		...
		&plan.Deployment.Trigger, &plan.Deployment.Status, &plan.Deployment.AgentID,
		&plan.Deployment.Payload, &plan.Deployment.Error,
	...
	for rows.Next() {
		...
		if item.Component.EnvSecretKeys == nil {
			item.Component.EnvSecretKeys = []string{}
		}
...
func (d *DB) GetDeployment(ctx context.Context, id string) (Deployment, error) {
	dep, err := scanDeployment(d.Pool.QueryRow(ctx,
		`SELECT `+deploymentSelectCols+`
		 FROM deployments d
		 JOIN deploy_projects p ON p.id = d.project_id
		 JOIN deploy_releases r ON r.id = d.release_uuid
...
func (d *DB) ListDeployments(ctx context.Context, limit int) ([]Deployment, error) {
	...
	var out []Deployment
	for rows.Next() {
		dep, err := scanDeployment(rows.Scan)
		if err != nil {
...
func (d *DB) ListDrainableDeployInstances(ctx context.Context) ([]DeployInstance, error) {
	return d.ListDrainableDeployInstancesForAgent(ctx, "")
}

// ListDrainableDeployInstancesForAgent narrows the drainable view to one
// agent (or central when agentID == "").
func (d *DB) ListDrainableDeployInstancesForAgent(ctx context.Context, agentID string) ([]DeployInstance, error) {
	var (
		filter string
		args   []any
	)
	...
	rows, err := d.Pool.Query(ctx,
		`SELECT i.id::text, i.component_id, p.slug, c.slug, COALESCE(i.release_uuid::text, ''), COALESCE(r.release_id, ''),
		 ...
		 ORDER BY i.drain_started_at NULLS FIRST`, args...)
	if err != nil {
...
func (d *DB) ListLiveManagedContainerIDs(ctx context.Context) (map[string]struct{}, error) {
	return d.ListLiveManagedContainerIDsForAgent(ctx, "")
}

// ListLiveManagedContainerIDsForAgent narrows the live-set to one agent
// (or central when agentID == "") so each deployer only reconciles its
// own Docker daemon.
func (d *DB) ListLiveManagedContainerIDsForAgent(ctx context.Context, agentID string) (map[string]struct{}, error) {
	var (
		filter string
		args   []any
	)
	...
	rows, err := d.Pool.Query(ctx,
		`SELECT i.container_id
		   FROM deploy_instances i
		   JOIN deploy_components c ON c.id = i.component_id
		  WHERE i.state IN ('active', 'draining') AND i.container_id != '' `+filter+`
		 UNION
		 SELECT i.container_id
		   FROM deploy_instances i
		   JOIN deploy_components c ON c.id = i.component_id
		   JOIN deployments dep ON dep.release_uuid = i.release_uuid
		  WHERE i.state = 'warming' AND i.container_id != '' AND dep.status = 'running' `+filter, args...)
	if err != nil {
...
func (d *DB) ListPrunableImageRefsForAgent(ctx context.Context, agentID string, componentID, keepN int) ([]string, error) {
	...
	var (
		filter string
		args   = []any{componentID, keepN}
	)
	if agentID == "" {
		filter = "AND c.agent_id IS NULL"
	} else {
		filter = "AND c.agent_id = $3"
		args = append(args, agentID)
	}
	rows, err := d.Pool.Query(ctx,
		`WITH keep AS (
...
//
// Scoped to the central deployer (NULL agent_id) — call the *ForAgent
// variant from edge deployers so a central crash doesn't reset edge work
// the agent is still running.
func (d *DB) ResetStaleRunningDeployments(ctx context.Context, olderThan time.Duration) (int, error) {
	return d.ResetStaleRunningDeploymentsForAgent(ctx, "", olderThan)
}

// ResetStaleRunningDeploymentsForAgent is the same recovery action but
// limited to deployments owned by a specific agent (or central when
// agentID == ""). Edge agents call this so they only roll back their
// own stuck work.
func (d *DB) ResetStaleRunningDeploymentsForAgent(ctx context.Context, agentID string, olderThan time.Duration) (int, error) {
	...
	tag, err := d.Pool.Exec(ctx,
		`UPDATE deployments
		 ...
		 WHERE status = 'running' AND updated_at < $1 `+filter, args...)
	if err != nil {
...
func (d *DB) CleanupStaleWarmingInstances(ctx context.Context) (int, error) {
	return d.CleanupStaleWarmingInstancesForAgent(ctx, "")
}

// CleanupStaleWarmingInstancesForAgent restricts the cleanup to one
// agent (or central when agentID == ""). Joins through deploy_components
// because instances themselves don't carry agent_id.
func (d *DB) CleanupStaleWarmingInstancesForAgent(ctx context.Context, agentID string) (int, error) {
	var (
		filter string
		args   []any
	)
	...
	tag, err := d.Pool.Exec(ctx,
		`UPDATE deploy_instances i
		 SET state = 'unhealthy', health_status = 'deployment terminated'
		 FROM deployments dep, deploy_components c
		 WHERE i.state = 'warming'
		   AND i.release_uuid = dep.release_uuid
		   AND i.component_id = c.id
		   AND dep.status IN ('failed', 'succeeded') `+filter, args...)
	if err != nil {
