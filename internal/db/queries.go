...
// normalizeTLSMode validates against the schema CHECK constraint, falling
// back to "auto" for unknown inputs rather than failing the request.
func normalizeTLSMode(mode string) string {
	switch mode {
	case "off", "redirect", "auto", "manual":
		return mode
	default:
		return "auto"
	}
...
func (d *DB) CreateHost(ctx context.Context, domain string, isActive, forceHTTPS bool, tlsMode string, trustedProxies []string, jwt HostJWT, target HostTarget) (Host, error) {
	...
	// Reconcile force_https with tls_mode so the two fields can never
	// disagree: redirect mode implies force_https; off mode forbids it.
	switch tlsMode {
	case "redirect":
		forceHTTPS = true
	case "off":
...
func (d *DB) GetLogStats(ctx context.Context, from, to time.Time, displayClaims []string) (LogStats, error) {
	...
	// vocabulary (one app might use user_id + org_id, another just user_id,
	// a SaaS might use email + sub, …). Empty list → skip the panel
	// entirely.
