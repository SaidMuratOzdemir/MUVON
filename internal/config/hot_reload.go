...
// Seed installs a pre-built Config without going through the source. The
// agent calls this when central is unreachable on cold start and a local
// cache exists, so the proxy can serve stale-but-working config while a
// background goroutine keeps polling for fresh state.
func (h *Holder) Seed(cfg *Config) error {
	if cfg == nil {
		return fmt.Errorf("config.Holder.Seed: nil config")
	}
	digest := configDigest(cfg)
	h.ptr.Store(cfg)
	h.lastDigest.Store(&digest)
