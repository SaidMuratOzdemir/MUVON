...
type Server struct {
	...
	clientEventPipeline *logger.ClientEventPipeline
	database            *db.DB
	configFn            ConfigFunc
...
//     (e.g. user_id for tenant A, sub for tenant B, ...). The previous
//     fallback to the global list silently dropped every tenant-specific
//     claim.
