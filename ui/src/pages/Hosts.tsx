...
import {
  ...
  ExternalLink, ShieldCheck, ShieldAlert, ShieldQuestion, AlertCircle,
} from 'lucide-react'
...
function TLSBadge({ host }: { host: Host }) {
  ...
  useEffect(() => {
    let cancelled = false
    api.getHostTLSStatus(host.id).then(s => { if (!cancelled) setStatus(s) }).catch(() => {})
    return () => { cancelled = true }
  }, [host.id])
  if (!status) return null
  const map: Record<api.HostTLSStatus['status'], { label: string; tone: string }> = {
    valid:     { label: `TLS · ${status.days_left}g`, tone: 'text-emerald-300 border-emerald-400/40' },
    expiring:  { label: `TLS · ${status.days_left}g`, tone: 'text-amber-300 border-amber-400/40' },
    expired:   { label: 'TLS · süresi doldu',          tone: 'text-destructive border-destructive/40' },
    missing:   { label: 'TLS · yok',                   tone: 'text-muted-foreground border-border' },
    off:       { label: 'TLS · kapalı',                tone: 'text-muted-foreground border-border' },
    wildcard:  { label: 'TLS · wildcard',              tone: 'text-muted-foreground border-border' },
  }
  const { label, tone } = map[status.status] ?? map.missing
  return (
    <Badge variant="outline" className={cn('text-[10px]', tone)} title={status.issuer ? `Issuer: ${status.issuer}` : undefined}>
      {label}
    </Badge>
  )
}
...
// ─── DNS Status Section ─────────────────────────────────────────────────────────
//
// Renders inside the expanded HostRow panel. Lazy: no fetch until the
// operator opens the row, no auto-refresh — the badge updates only on
// the explicit "tekrar dene" click. Lazy by design: a DNS lookup is a
// real network call we don't want to fan out for every row on mount.

function DNSStatusSection({ host }: { host: Host }) {
  const [status, setStatus] = useState<api.DNSStatus | null>(null)
  ...
  const run = useCallback(async () => {
    ...
    setErr(null)
    try {
      const r = await api.getHostDNSStatus(host.id)
      setStatus(r)
    } catch (e) {
  ...
  const { icon, tone, title, detail } = (() => {
    if (loading) return { icon: <Loader2 className="h-4 w-4 animate-spin" />, tone: 'text-muted-foreground', title: 'DNS kontrol ediliyor…', detail: '' }
    if (err) return { icon: <AlertCircle className="h-4 w-4 text-destructive" />, tone: 'text-destructive', title: 'DNS kontrolü başarısız', detail: err }
    ...
    switch (status.status) {
      ...
      case 'wildcard': return { icon: <ShieldQuestion className="h-4 w-4" />, tone: 'text-muted-foreground', title: 'Wildcard host', detail: 'Her alt alan adı kendi başına doğrulanır.' }
      default: return { icon: <AlertCircle className="h-4 w-4 text-destructive" />, tone: 'text-destructive', title: 'DNS hatası', detail: status.detail ?? '' }
    }
  })()
    ...
    <div className="px-4 py-3 border-b border-border/40 bg-muted/10">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className={cn('text-sm font-medium', tone)}>{title}</div>
...
export default function Hosts() {
  ...
  const [hostForm, setHostForm] = useState({
    ...
    is_active: true,
    force_https: false,
    tls_mode: 'auto' as 'off' | 'redirect' | 'auto' | 'manual',
  ...
  function openAddHost() {
    setHostForm({
      domain: '', is_active: true, force_https: false, tls_mode: 'auto', trusted_proxies: [],
        ...
        <DialogContent className="bg-card border-border max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">TLS modu</Label>
              <select
                value={hostForm.tls_mode}
                onChange={e => {
                  // Mode dictates the listener behaviour; keep force_https
                  // in sync with the obvious choices.
                  const v = e.target.value as 'off' | 'redirect' | 'auto' | 'manual'
                  setHostForm(f => ({
                    ...f,
                    tls_mode: v,
                    force_https: v === 'redirect' ? true : v === 'off' ? false : f.force_https,
                  }))
                }}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
              >
                <option value="auto">Otomatik (Let's Encrypt)</option>
                <option value="manual">Manuel (yüklenmiş sertifika)</option>
                <option value="redirect">Yalnız yönlendirme (HTTP → HTTPS)</option>
                <option value="off">Kapalı (HTTP-only)</option>
              </select>
              <p className="text-[11px] text-muted-foreground">
                <code className="font-mono">auto</code> Let's Encrypt'ten otomatik alır.
                {' '}<code className="font-mono">manual</code> yalnız panelden yüklenen sertifikayı kullanır.
                {' '}<code className="font-mono">off</code> ise :443 dinlemez.
              </p>
            </div>
            <TagInput
