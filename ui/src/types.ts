...
export interface Host {
  ...
  is_active: boolean;
  force_https: boolean;
  // TLS strategy chosen by the operator:
  //   off      — HTTP only, no listener on :443
  //   redirect — 301 from HTTP to HTTPS (implies force_https)
  //   auto     — Let's Encrypt ACME (default)
  //   manual   — admin-uploaded cert in tls_certificates; no ACME attempt
  tls_mode: 'off' | 'redirect' | 'auto' | 'manual';
...
export interface Agent {
  id: string;
  name: string;
  // Plaintext key is only available in the create response (one-shot
  // reveal). List/get endpoints omit this field, so callers must treat
  // it as optional.
  api_key?: string;
  is_active: boolean;
...
export interface DeployComponent {
  ...
  // Names the keys in `env` whose values are stored encrypted server-side.
  // GET responses return `********` for these; an update keeps the existing
  // ciphertext when the same `********` value is resubmitted.
  ...
  // clearing the flag.
  paused: boolean;
  // Cap on recent succeeded releases whose images stay on the host. After
